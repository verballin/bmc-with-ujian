...
<!-- Header End -->
    ...
    <div class="container py-5">
        ...
        <div class="d-flex justify-content-between mb-3">
            <div><strong>Durasi Ujian:</strong> <?= esc($pengaturan['waktu']) ?> menit</div>
            <div><strong>Waktu Tersisa:</strong> <span id="timer" class="text-danger fw-bold"></span></div>
        </div>

        <form action="<?= site_url('pengaturanujian/simpanjawaban/' . $pengaturan['id_pengaturan']) ?>" method="post">
                ...
                <div class="card mb-4">
                    <div class="card-body">
                        ...
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="jawaban[<?= $s['soal_id'] ?>]" value="a" id="soal<?= $s['soal_id'] ?>a">
                            <label class="form-check-label" for="soal<?= $s['soal_id'] ?>a">A. <?= $s['a'] ?></label>
                        </div>

                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="jawaban[<?= $s['soal_id'] ?>]" value="b" id="soal<?= $s['soal_id'] ?>b">
                            <label class="form-check-label" for="soal<?= $s['soal_id'] ?>b">B. <?= $s['b'] ?></label>
                        </div>

                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="jawaban[<?= $s['soal_id'] ?>]" value="c" id="soal<?= $s['soal_id'] ?>c">
                            <label class="form-check-label" for="soal<?= $s['soal_id'] ?>c">C. <?= $s['c'] ?></label>
                        </div>

                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="jawaban[<?= $s['soal_id'] ?>]" value="d" id="soal<?= $s['soal_id'] ?>d">
                            <label class="form-check-label" for="soal<?= $s['soal_id'] ?>d">D. <?= $s['d'] ?></label>
                        </div>
                    </div>
                </div>
            <?php endforeach; ?>
            ...
            <div class="text-end">
                <button type="submit" class="btn btn-success btn-lg rounded-pill">Kumpulkan Jawaban</button>
            </div>
        </form>
    </div>
    ...
    <script>
        // Timer Ujian
        let waktu = <?= $pengaturan['waktu'] ?> * 60;
        const timerEl = document.getElementById('timer');

        const updateTimer = () => {
            const menit = Math.floor(waktu / 60);
            const detik = waktu % 60;
            timerEl.textContent = `${menit}:${detik < 10 ? '0' + detik : detik}`;
